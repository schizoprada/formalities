# ~/formalities/sandbox/llama_char_count.py
import ollama
...
def main():
    ...
    # First interaction
    print("\n=== Initial Response ===")
    response = ollama.chat(model='llama3.2', messages=messages, tools=[mmtool, mbtool])
    print(f"{response}\n")
        ...
        for tool in calls:
            if (func := FUNCS.get(tool.function.name)):
                ...
                if tool.function.name == 'mmtool':
                    print(f"Result: {result[:100]}...")  # Truncate long mmtool results
                else:
